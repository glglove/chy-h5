<template>
	...
	>
		<view class="page login" id="login" slot="container-slot">
			<view class="content">
				<!-- 头部logo -->
				<view class="header">
	...
	export default {
		...
		data() {
			return {
				...
				toPageUrl: '/pages/find/find', // 登陆成功后跳转的页面
				jumpType: 'switch',  // 跳转路由的方式:navigate、switch、relanch、redirect
			};
		},
		components:{
			wInput,
		...
		methods: {
		    ...
		    startLogin(){
				...
				if (this.userData.UserName.length == "") {
					this.alert("用户名不能为空", 2000, false)
				    return;
				}
		        if (this.userData.pwd.length < 5) {
					this.alert("密码不正确", 2000, false)
		            return;
		        }
